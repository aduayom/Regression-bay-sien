...
# le sigma2beta est considéré comme un hyperparamètre
# on considère que mu = 0 connue 
...
########################################################################
# Question 3 
# On peut utiliser le programe précédent simplement en changeant la variance
# s2beta est remplacée par c s2eps solve(t(X)%*%X)
s2eps=4 # supposée connue 
...
plot(sort(abs(rowMeans(res))))
# prenons un paramètre ridge plus important 
ridge=10 # paramètre ridge
p=300 
identite=diag(1,p,p)
sridge= c*s2eps*solve(t(simuvarexpl)%*%simuvarexpl + ridge*identite)
nburn=10
...
# il faudrait extraire une valeur sur 10 (par ex) pour rendre les données 
# moins dépendantes 
plot(density(resbeta[50,]))
########################################################################
########################################################################
...
### initialisation
beta <- rep(0,p) # car les beta centrées 
...
# 2) prendre une simu sur 10 (pour diminuer la dépdendance) et avoir 
